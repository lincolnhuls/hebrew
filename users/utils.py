...
"""
    ...
    if not _send_mail_safe(subject, body, [user.email]):
        logger.warning("Confirmation email was not delivered to %s", user.email)
...
Hebrew for Everyone
"""
    if _send_mail_safe(subject, body, [user.email]):
