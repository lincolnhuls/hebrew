...
def chapter_view(request, book_slug, chapter_number):
    ...
    book = get_object_or_404(BibleBook, slug=book_slug)
    chapter = get_object_or_404(BibleChapter.objects.select_related('book'), book=book, number=chapter_number)
    ...
    prev_in_book = BibleChapter.objects.filter(book=book, number__lt=chapter_number).select_related('book').order_by("-number").first()
    if prev_in_book:
        prev_chapter = prev_in_book
    else:
        prev_book = BibleBook.objects.filter(order__lt=book.order).order_by("-order").first()
        if prev_book:
            last_ch = BibleChapter.objects.filter(book=prev_book).select_related('book').order_by("-number").first()
            prev_chapter = last_ch

    # Next chapter: same book next, or next book's first chapter
    next_in_book = BibleChapter.objects.filter(book=book, number__gt=chapter_number).select_related('book').order_by("number").first()
    if next_in_book:
        next_chapter = next_in_book
    else:
        next_book = BibleBook.objects.filter(order__gt=book.order).order_by("order").first()
        if next_book:
            first_ch = BibleChapter.objects.filter(book=next_book).select_related('book').order_by("number").first()
